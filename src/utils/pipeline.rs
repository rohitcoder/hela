...
pub async fn pipeline_failure(
    ...
    if is_sast {
        ...
        for result in sast_results {
            ...
            let is_hashed_message_exists =
                common::check_hash_exists(&hashed_message, &mogno_uri).await;
            if !is_hashed_message_exists {
                ...
                table.add_row(row![
                    ...
                    result["path"],
                    result["severity"],
                    result["message"].chars().take(50).collect::<String>()
                ]);
    ...
    if is_sast {
        ...
        for result in json_output["sast"].as_array().unwrap() {
            ...
            tags.push(Value::String(
                commiter_info["email"].to_string().replace("\"", ""),
            ));
            if !commit_id.is_empty() {
